...
type
  MersenneTwister* = ref object of Generator
    idx: uint
...
proc fillNextState*(mt: MersenneTwister) {.inline.} =
  ...
  for i in 0..NN - MM - 1:
    ...
    mt.mtState[i] =
      cast[uint64](mt.mtState[i + MM].uint64 xor (x shr 1).uint64 xor (
        (x and 1) * MATRIX_A
      ))

  for i in NN - MM..NN - 2:
    let x = (mt.mtState[i] and UM).uint64 or (mt.mtState[i + 1].uint64 and LM)

    mt.mtState[i] =
      cast[uint64](mt.mtState[i + MM - NN].uint64 xor (x shr 1).uint64 xor (
        (x and 1).uint64 * MATRIX_A
      ))

  let x = (mt.mtState[NN - 1] and UM).uint64 or (mt.mtState[0] and LM)
  mt.mtState[NN - 1] =
    cast[uint64](mt.mtState[MM - 1] xor (x shr 1) xor
      ((x and 1).uint64 * MATRIX_A).uint64)
  mt.idx = 0
...
proc newMersenneTwister*(seed: uint64): MersenneTwister {.inline.} =
  ...
  for i in 1..NN - 1:
    state[i] =
      6364136223846793005'u64 * (state[i - 1] xor (state[i - 1] shr 62)) +
      cast[uint64](i)

  MersenneTwister(idx: NN, seed: seed, mtState: state)
